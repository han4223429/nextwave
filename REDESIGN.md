...
- **3D `NEXTWAVE` 히어로** — Three.js로 글자 하나하나를 입체 모델링. 시간·스크롤·마우스 움직임에 따라 글자가 제각각 회전합니다. (데스크톱 1줄 / 모바일 2줄 자동 전환, WebGL 미지원 시 그라데이션 텍스트로 자동 폴백)
...
## 🎨 3D 글자 커스터마이즈 (`three-hero.js`)

- 색상: `palette` 배열 (글자별 색).
- 회전 속도: `loop()`의 `u.speed`, 스크롤 반응량 `scroll * 0.0045`.
- 글자 크기/두께: `TextGeometry`의 `size` / `height` / `bevel*`.
- 모바일 2줄 전환 기준: `layout()`의 `w < 640`.
